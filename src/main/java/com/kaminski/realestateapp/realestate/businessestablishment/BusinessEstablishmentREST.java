...
@RestController
@RequestMapping(value = "/business-establishment", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
public class BusinessEstablishmentREST {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<BusinessEstablishment> getFlatById(@PathVariable Long id) {
        ...
        if (businessEstablishment.isPresent()) {
            return ResponseEntity.ok().body(businessEstablishment.get());
        } else {
